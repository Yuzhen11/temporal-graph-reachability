...
bool query(int from, int to)
{
	...
	*/
	...
	for (int i = 0; i < v[from].size(); ++ i)
	{
		...
		if (reachindex[w].Ldown<reachindex[to].Ldown/*&&reachindex[w].Lup>reachindex[to].Lup*/ && flag[w]!=mark)
		{
...
//----------------------update---------------------------
bool mergeLout(const int u1, const int u2)
{
	vector<pair<int, int> > ans;
	int p1 = 0; int p2 = 0;
	bool change = 0;
	while(p1 < Lout[u1].size() || p2 < Lout[u2].size())
	{
		if (ans.size() == k) continue;
		if (p1 == Lout[u1].size())
		{
			change = 1;
			ans.push_back(Lout[u2][p2]);
			p2++;
			continue;
		}
		if (p2 == Lout[u2].size())
		{
			ans.push_back(Lout[u1][p1]);
			p1++;
			continue;
		}
		if (Lout[u1][p1].first < Lout[u2][p2].first)
		{
			ans.push_back(Lout[u1][p1]);
			p1++;
		}
		else if (Lout[u1][p1].first > Lout[u2][p2].first)
		{
			change = 1;
			ans.push_back(Lout[u2][p2]);
			p2++;
		...
		{
			if (Lout[u1][p1] > Lout[u2][p2]) change = 1;
			ans.push_back(min(Lout[u1][p1], Lout[u2][p2]));
			p1++; p2++;
		}
	}
	Lout[u1] = ans;
...
bool mergeLin(const int u1, const int u2)
{
	vector<pair<int, int> > ans;
	int p1 = 0; int p2 = 0;
	bool change = 0;
	while(p1 < Lin[u1].size() || p2 < Lin[u2].size())
	{
		if (ans.size() == k) continue;
		if (p1 == Lin[u1].size())
		{
			change = 1;
			ans.push_back(Lin[u2][p2]);
			p2++;
			continue;
		}
		if (p2 == Lin[u2].size())
		{
			ans.push_back(Lin[u1][p1]);
			p1++;
			continue;
		}
		if (Lin[u1][p1].first < Lin[u2][p2].first)
		{
			ans.push_back(Lin[u1][p1]);
			p1++;
		}
		else if (Lin[u1][p1].first > Lin[u2][p2].first)
		{
			change = 1;
			ans.push_back(Lin[u2][p2]);
			p2++;
		...
		{
			if (Lin[u1][p1] < Lin[u2][p2]) change = 1;
			ans.push_back(max(Lin[u1][p1], Lin[u2][p2]));
			p1++; p2++;
		}
	}
	Lin[u1] = ans;
...
void addEdge(const int u1, const int u2)
{
	//maintain CR
	//Lout
	queue<int> q;
	if (mergeLout(u1, u2)) q.push_back(u1);
	while(!q.empty())
	{
		int tmp = q.front();
		q.pop();
		for (int i = 0; i < vr[tmp].size(); ++ i)
		{
			if (mergeLout(vr[tmp][i], tmp)) q.push(vr[tmp][i]);
		}
	}
	//Lin
	queue<int> q1;
	if (mergeLin(u2, u1)) q1.push_back(u2);
	while(!q.empty())
	{
		int tmp = q1.front();
		q1.pop();
		for (int i = 0; i < v[tmp].size(); ++ i)
		{
			if (mergeLin(v[tmp][i], tmp)) q1.push(v[tmp][i]);
		}
	}
	
	//update topological level
	if (Ldown[u1]<=Ldown[u2])
	{
		int addLevel = Ldown[u2]-Ldown[u1]+1;
		updateFlag[u2] = updateMark;
		Ldown[u2] += addLevel;
		queue<int> qt;
		qt.push_back(u2);
		while(!qt.empty())
		{
			int tmp = qt.front(); qt.pop();
			for (int i = 0; i < v[tmp].size(); ++ i)
			{
				if (updateFlag[v[tmp][i]] != updateMark)
				{
					qt.push_back(v[tmp][i]);
					updateFlag[v[tmp][i]] = updateMark;
					Ldown[v[tmp][i]] += addLevel;
				}
			}
		}
	}
...
int main(int argc, char* argv[])
{
	...
	//n*k*2
	cout << "INDEX_SIZE= " << (count1 + n*2 + n*2)*4.0/1024/1024 << " MB" << endl;
	
	
	//update flag
	updateMark = 0;
	updateFlag = new int[n];
	memset(updateFlag, 0, sizeof(int)*n);	
}
